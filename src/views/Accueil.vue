<template>
	<div id="page">
		<div id="accueil" :style="{'background-image': 'url(./static/img/digiflashcards.jpg)'}">
			<div id="langues">
			...
			<div id="conteneur">
				<div id="contenu">
					<h1>
						<span>Digiflashcards</span> <span>by La Digitale</span>
					</h1>
					<div>
						<p v-html="$t('slogan')" />
				...
				<div id="credits">
					...
					<p>{{ new Date().getFullYear() }} - <a href="https://ladigitale.dev" target="_blank" rel="noreferrer">La Digitale</a> - <a href="https://gitlab.com/ladigitale/digiflashcards" target="_blank" rel="noreferrer">{{ $t('codeSource') }}</a> - <span class="hub" @click="ouvrirHub"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#001d1d" width="36px" height="36px"><path d="M0 0h24v24H0z" fill="none"/><path d="M4 8h4V4H4v4zm6 12h4v-4h-4v4zm-6 0h4v-4H4v4zm0-6h4v-4H4v4zm6 0h4v-4h-4v4zm6-10v4h4V4h-4zm-6 4h4V4h-4v4zm6 6h4v-4h-4v4zm0 6h4v-4h-4v4z"/></svg></span></p>
				</div>
			</div>
		</div>
	...
	methods: {
		...
		creer () {
			if (this.nom !== '' && this.question !== '' && this.reponse !== '') {
				...
				xhr.onload = function () {
					if (xhr.readyState === xhr.DONE && xhr.status === 200) {
						...
						if (xhr.responseText !== 'erreur') {
							this.$router.push('/f/' + xhr.responseText)
						} else {
				...
				xhr.open('POST', this.$parent.$parent.hote + 'inc/creer_serie.php', true)
